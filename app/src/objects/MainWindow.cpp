...
void MainWindow::Render(int particleCount) {
  ...
  if (renderTex_ != 0) {
    ...
    ImGui::Image((void *)(intptr_t)renderTex_, imgSize, ImVec2(0, 1),
                 ImVec2(1, 0));
  }
  ...
  ImGui::PopItemWidth();
  if (gravity_ != prevG && onGravityChanged_)
    onGravityChanged_(gravity_);

  float prevV = viscosity_;
  ImGui::PushItemWidth(160.f);
  ImGui::SliderFloat("Viscosity", &viscosity_, 0.0f, 5.0f, "%.2f");
  ImGui::PopItemWidth();
  if (viscosity_ != prevV && onViscosityChanged_)
    onViscosityChanged_(viscosity_);
  ...
  if (quality_ != prevQ && onQualityChanged_)
    onQualityChanged_(quality_);

  float prevRad = renderRadius_;
  ImGui::PushItemWidth(160.f);
  ImGui::SliderFloat("Particle size", &renderRadius_, 0.022f, 0.05f, "%.3f");
  ImGui::PopItemWidth();
  if (renderRadius_ != prevRad && onRenderRadiusChanged_)
    onRenderRadiusChanged_(renderRadius_);

  float prevCol[3] = {color_[0], color_[1], color_[2]};
  ImGui::ColorEdit3("Fluid color", color_);
  if ((color_[0] != prevCol[0] || color_[1] != prevCol[1] ||
       color_[2] != prevCol[2]) &&
      onColorChanged_) {
