...
// ---------------------------------------------------------------------------
// SPH kernel functions (2-D, Müller et al. 2003)
// ---------------------------------------------------------------------------
...
glm::vec2 FluidSim::SpikyGrad(glm::vec2 r_vec, float r_len) const {
    if (r_len <= 0.0f || r_len >= h_) return glm::vec2(0.0f);
    float f = h_ - r_len;
...
FluidSim::~FluidSim() {
    ...
    if (sceneVAO_)     glDeleteVertexArrays(1, &sceneVAO_);
    if (sceneVBO_)     glDeleteBuffers(1, &sceneVBO_);
}
...
void FluidSim::ComputeForces() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            if (r_len >= h_ || r_len < 1e-6f) continue;
            ...
            // Viscosity force
            fv += viscosity_ * mass_ *
                  (particles_[j].vel - particles_[i].vel) / particles_[j].density *
                  ViscLaplacian(r_len);
        }
        // Gravity
...
glm::vec2 FluidSim::ClosestOnSegment(glm::vec2 p, glm::vec2 a, glm::vec2 b) const {
    glm::vec2 ab = b - a;
...
void FluidSim::ResolveObstacle(Particle& p) {
    glm::vec2 edges[3][2] = {{obstA_, obstB_}, {obstB_, obstC_}, {obstC_, obstA_}};
    for (auto& edge : edges) {
        ...
        if (dist < renderRadius_ * 1.5f && dist > 1e-6f) {
            ...
            // Reflect velocity along normal and dampen
...
void FluidSim::SpawnParticles(float dt) {
    if ((int)particles_.size() >= maxParticles_) return;
    spawnTimer_ += dt;
...
void FluidSim::InitParticleGL() {
    const int segs = 14;
    circleVerts_ = segs + 2; // center + segs + closing vertex

    std::vector<glm::vec2> circle;
    circle.reserve(circleVerts_);
    circle.push_back({0.0f, 0.0f}); // center
    ...
    glBufferData(GL_ARRAY_BUFFER, maxParticles_ * sizeof(glm::vec3), nullptr, GL_DYNAMIC_DRAW);
...
void FluidSim::UpdateInstanceBuffer() {
    ...
    for (int i = 0; i < n; ++i) {
        float speed01 = glm::clamp(glm::length(particles_[i].vel) / maxSpeed, 0.0f, 1.0f);
...
void FluidSim::RenderParticles(GLuint program, GLint uRadius, GLint uColorLow, GLint uColorHigh) {
    int n = (int)particles_.size();
    if (n == 0) return;

    glUseProgram(program);
    if (uRadius >= 0)    glUniform1f(uRadius, renderRadius_);
    if (uColorLow >= 0)  glUniform3fv(uColorLow,  1, &baseColor_[0]);
    // High-speed color: shifted toward white
    glm::vec3 highColor = glm::mix(baseColor_, glm::vec3(1.0f, 0.95f, 0.85f), 0.85f);
    if (uColorHigh >= 0) glUniform3fv(uColorHigh, 1, &highColor[0]);
...
// ---------------------------------------------------------------------------
// GL: static scene geometry (walls + obstacle + source indicator)
// ---------------------------------------------------------------------------

void FluidSim::InitSceneGL() {
    // Each quad = 6 floats (2 triangles, each float is x or y)
    // Layout: [walls(24 verts)] [obstacle(3 verts)] [source(6 verts)]
    // We store vec2 only (z=0 is implicit)

    const float il = wallL_, ir = wallR_, ib = wallB_, it = wallT_;
    const float tw = 0.05f; // wall thickness

    // Helper: push a quad (2 triangles) as 6 vec2
    ...
    // Walls (24 vertices = 4 quads * 6 verts)
    pushQuad(verts, il-tw, ib-tw, il,    it+tw); // left
    pushQuad(verts, ir,    ib-tw, ir+tw, it+tw); // right
    pushQuad(verts, il-tw, ib-tw, ir+tw, ib);    // bottom
    pushQuad(verts, il-tw, it,    ir+tw, it+tw); // top

    // Obstacle triangle (3 vertices)
    ...
    // Source indicator rectangle (6 vertices = 1 quad)
...
void FluidSim::RenderScene(GLuint program, GLint uColor) {
    ...
    // Walls – dark slate
    if (uColor >= 0) glUniform4f(uColor, 0.25f, 0.28f, 0.33f, 1.0f);
    glDrawArrays(GL_TRIANGLES, 0, 24);

    // Obstacle – lighter slate
    if (uColor >= 0) glUniform4f(uColor, 0.42f, 0.45f, 0.52f, 1.0f);
    glDrawArrays(GL_TRIANGLES, 24, 3);

    // Source indicator – teal glow
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    if (uColor >= 0) glUniform4f(uColor, 0.10f, 0.85f, 0.75f, 0.55f);
